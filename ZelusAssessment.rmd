...
start_vals = data.frame(k = 10, s= 10)
...
inning_data_use %>% filter(matchid==238214 &over=="2.1")
...
inning_data_use <- inning_data_use %>% 
  ...
  mutate(over_number = floor(as.numeric(over)),
         ## add in a binary for if a wicket occured to be able to calculate remaining wickets.  
         is_wicket = ifelse(is.na(wicket.kind), 0, 1)) %>% 
  arrange(matchid, innings, over) %>% 
...
runs_per_over <- inning_data_use %>% 
  ...
  mutate(start_wickets_remaining = max(wickets_remaining), 
         total_runs = sum(runs.total)) %>%  ## get the wickets remaining at the start of the over. 
  select(matchid, innings, over_number, overs_remaining, start_wickets_remaining, total_runs) %>% 
    ungroup() %>% 
  distinct()
...
runs_per_over_avg <- runs_per_over  %>% 
  ...
  summarise(total_runs = mean(total_runs), total = n())
