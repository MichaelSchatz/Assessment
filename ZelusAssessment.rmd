...
for(i in 1:reps) {
 ...
 if(nrow(check_vals_use)==0) {
   stop("Reached optimal integer value, check results")
 }
...
```{r q4_dev, echo=TRUE, include = TRUE}
library(mgcv)
...
Using the intercept and innings values for the model we can derive the "average team's" expected run per over. For inning 1 the average expected runs per over is `r exp(fit$coefficients[1])` runs and in inning 2 the average expected runs per over is `r exp(fit$coefficients[1] + fit$coefficients[2])` runs. Averaging those two together (Acknowledging that there are likely more actual overs in inning 1, but for the purpose of this I believe we should ignore that assumption), we get an estimate of `r (exp(fit$coefficients[1]) + (exp(fit$coefficients[1] + fit$coefficients[2])))/2 ` Runs per over. 
...
```{r q5_kable  echo=FALSE, include = TRUE}
